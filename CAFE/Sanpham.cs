...
namespace CAFE
{
    public partial class Sanpham : Form
    {
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            sql = $"SELECT masanpham FROM sanpham WHERE masanpham = N'{txtmasp.Text.Trim()}'";
            if (CLass.Functions.CheckKey(sql))
            {
                MessageBox.Show("Mã thú này đã có, bạn phải nhập mã thú khác", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtmasp.Focus();
                txtmasp.Text = "";
                return;
            }
...
$"(N'{txtmasp.Text.Trim()}',N'{txttensp.Text.Trim()}',N'{cbbmaloai.SelectedValue}',N'{txtdongiaban.Text.Trim()}',N'{txtsoluong.Text.Trim()}',N'{txtanh.Text.Trim()}',N'{cbbmacongdung.SelectedValue}',N'{txtdongianhap.Text.Trim()}')";
        ...
        {
            string sql;
            sql = $"SELECT masanpham FROM chitiethdb WHERE masanpham=N'{txtmasp.Text.Trim()}'";
